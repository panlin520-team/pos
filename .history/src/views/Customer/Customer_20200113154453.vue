      ...
      <div class="CustRight" v-show="moreClick == false">
        <div v-show="num==0" class="personage">
          <div class="rightTitle">
            ...
            <PopOver
              ...
              width="500px"
            >
              <div class="stgblcktops" slot="top">修改密码</div>
              <div class="stgblcktopmains" slot="main">
                <el-form
                  :model="ruleForm"
                  status-icon
                  :rules="rules"
              ...
              <div class="stgblblbottoms" slot="bottom">
                <el-button size="mini" type="primary" @click="submitFormpassword('ruleForm')">确定修改</el-button>
                <el-button size="mini" @click="resetFormpassword('ruleForm')">重置</el-button>
              </div>
            </PopOver>
          </div>
        ...
        <div v-show="num==2" class="ItemInformation">
          ...
          <!-- <el-table :data="savingsRecordFilter" class="el-tableList">
            ...
            <el-table-column prop="amount" label="充值金额" width="100"></el-table-column>
            <el-table-column prop="accountTypeName" label="充值类型"></el-table-column>
            <el-table-column prop="cardNumber" label="卡号"></el-table-column>
            <el-table-column prop="createTime" label="创建时间"></el-table-column>
          </el-table>-->
          <PopOver
            custom-class="storagebsalesR"
            :visible.sync="visible_salesR"
            @close="closeTimesalesR"
            width="450px"
          >
            <div class="stgblcktop" slot="top">账户退款</div>
            <div class="stgblcktopmain" slot="main">
              <div class="optionDate">
                <label>退款金额：</label>
                <el-input v-model="input_mesalesR" placeholder="请输入退款金额"></el-input>
              </div>
            </div>
            <div class="stgblckbottom" slot="bottom">
...
export default {
  ...
  methods: {
            ...
            dataView: {
              ...
              optionToContent: function(opt) {
                ...
                series.forEach(function(item) {
                  //组装表头
                  tdHeads += `<td style="padding: 10px 50px">${item.name}</td>`;
                });
          ...
          feature: {
            ...
            dataView: {
              ...
              optionToContent: function(opt) {
                let axisData = opt.xAxis[0].data; //坐标数据
                let series = opt.series; //折线图数据
                let tdHeads = '<td  style="padding: 10px">时间</td>'; //表头
                let tdBodys = ""; //数据
                series.forEach(function(item) {
                  //组装表头
                  tdHeads += `<td style="padding: 10px 50px">${item.name}</td>`;
          ...
          feature: {
            ...
            dataView: {
              ...
              optionToContent: function(opt) {
                let axisData = opt.xAxis[0].data; //坐标数据
                let series = opt.series; //折线图数据
                let tdHeads = '<td  style="padding: 10px">项目</td>'; //表头
                let tdBodys = ""; //数据
                series.forEach(function(item) {
                  //组装表头
    ...
    //退款成功后
    savingAccotrue() {
      var url = this.$https.orderHost + "/order/rechargeOrderRefund";
      var params = {
        orderNumber: this.orderNumbes,
        refundAmount: this.input_mesalesR,
        name: this.orderssname,
        mobile: this.mobiletple,
        memberNum: this.memberNus,
        accountTypeId: this.accountswer
      };
      this.$https.fetchPost(url, params).then(res => {
        if (res.data.responseStatusType.message == "Success") {
          this.$message({
