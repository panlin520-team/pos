...
<template>
  <div class="employeesPage fluid">
      ...
      <div class="list scrollY">
        ...
        <div class="listItem" v-for="(item,index) in emlpoyeesDataFilter" :key="index">
          ...
          <div>
            ...
            <el-button size="mini" @click="salaryname(item)" type="primary">工资</el-button>
          </div>
        </div>
        <!-- 分页 -->
    ...
    <PopOver
      ...
      width="450px"
    >
      <div class="stgblcktop" slot="top">工资时间段</div>
    ...
    <!-- 工资显示框 -->
    <PopOver
      custom-class="storageblocksalary"
      :visible.sync="visible_salary"
      @close="closeTsalary"
      width="450px"
    >
      <div class="stgblcktop" slot="top">工资详情</div>
      <div class="stgblcktopmain" slot="main">
        <div>
          <el-button @click="performancetrue" size="small" type="success">业绩汇总</el-button>
          <el-button @click="performancefalse" size="small" type="success">业绩明细</el-button>
        </div>
        ...
        <div class="newcenters">
          <label>员工编号</label>
          <div>{{newdatasalary.salesmanID}}</div>
        </div>
        <div class="newcenters">
          <label>开始时间</label>
          <div>{{newdatasalary.statisticDateStart}}</div>
        </div>
        <div class="newcenters">
          <label>结束时间</label>
          <div>{{newdatasalary.statisticDateEnd}}</div>
        </div>
        <div class="newcenters">
          <label>个数业绩</label>
          <div>{{newdatasalary.numberPerformance}}</div>
        </div>
        <div class="newcenters">
          <label>金额业绩</label>
          <div>{{newdatasalary.amountPerformance}}</div>
        </div>
        <div class="newcenters">
          <label>分数</label>
          <div>{{newdatasalary.score}}</div>
        </div>
        <div class="newcenters">
          <label>底薪</label>
          <div>{{newdatasalary.baseSalary}}</div>
        </div>
        <div class="newcenters">
          <label>提成</label>
          <div>{{newdatasalary.takePercentage}}</div>
        </div>
        <div class="newcenters">
          <label>工资</label>
          <div>{{newdatasalary.salary}}</div>
        </div>
      </div>
      <div class="stgblckbottom" slot="bottom">
        <el-button @click="salary_true" size="small" type="success">确定</el-button>
        <el-button @click="salary_false" size="small" type="info">取消</el-button>
      </div>
    </PopOver>
    <pop-over
    ...
    <!-- 业绩汇总弹出框 -->
    <PopOver
      custom-class="storageblocdetail"
      :visible.sync="visible_formancetrue"
      @close="closeformancetrue"
      width="450px"
    >
      <div class="stgblcktop" slot="top">业绩汇总</div>
      <div class="stgblcktopmain" slot="main">
        <el-table :data="tableexamine" style="width: 100%">
          <el-table-column label="业绩名称">
            <template slot-scope="scope">
              <span>{{ scope.row.achievementPostName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="业绩个数">
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">{{ scope.row.amountOrNumber }}</div>
            </template>
          </el-table-column>
        </el-table>
      </div>
          ...
          <div class="pagingbanner">
            <div class="block">
              <el-pagination
                ...
                :current-page.sync="currentPagefor"
                :page-size="pageSizefor"
                background
                layout="total, prev, pager, next"
                :total="totalSizesfor"
              ></el-pagination>
    ...
    <PopOver
      custom-class="storageblocdetail"
      :visible.sync="visible_formancedetail"
      @close="closeformancedetail"
      width="600px"
    >
      <div class="stgblcktop" slot="top">业绩明细</div>
      <div class="stgblcktopmain" slot="main">
        <el-table :data="tableexadetail" style="width: 100%">
          <el-table-column label="业绩明细ID">
            <template slot-scope="scope">
              <span>{{ scope.row.productName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="业绩名称" width="150">
            <template slot-scope="scope">
              <span>{{ scope.row.achievementPostName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="业绩金额">
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">{{ scope.row.ladderDetailedAmount }}</div>
            </template>
          </el-table-column>
          <el-table-column label="业绩个数">
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">{{ scope.row.ladderDetailedNumber }}</div>
            </template>
          </el-table-column>
          <el-table-column label="生成时间" width="150">
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">{{ scope.row.ladderDetailedDatetime }}</div>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div class="stgblckbottom" slot="bottom">
        <div class="paging">
          <div class="pagingbanner">
            <div class="block">
              <el-pagination
                @size-change="handleSizeChangetal"
                @current-change="handleCurrentChangetal"
                :current-page.sync="currentPagetal"
                :page-size="pageSizetal"
                background
                layout="total, prev, pager, next"
                :total="totalSizestal"
              ></el-pagination>
...
export default {
  ...
  data() {
    return {
      ...
      // 成员详细信息
      employeesOpen: false,
      visible_formancetrue: false,
      visible_formancedetail: false,
      // 编辑器
      editorPop: false,
      visible_times: false,
      visible_salary: false,
      content: null,
      //时间
      value_invenT: "",
      cordename: "",
      ...
      newdatasalary: {
        ...
        identical: "312313412",
        onjed: "5000",
        statisticDateStart: "2019.02.03",
        statisticDateEnd: "2020.03.02"
      },
      // 页码控制
      // 当前页码
      currentPage: 1,
  ...
  methods: {
    ...
    salaryname(res) {
      console.log(res);
      this.namespeopleId = res.staffNumber;
      this.cordename = res.name;
      this.visible_times = true;
    },
    //业绩汇总
    performancetrue() {
      this.looksalarcollect();
      this.visible_formancetrue = true;
    },
    closeformancetrue() {
      this.visible_formancetrue = false;
    },
    closeformancedetail() {
      this.visible_formancedetail = false;
    },
    performancefalse() {
      this.looksalardetail();
      this.visible_formancedetail = true;
    },
    //选择日期确定
    confirm_true() {
      if (this.value_invenT) {
        this.visible_salary = true;
        this.looksalary();
      } else {
        this.$message({
          type: "warning",
    ...
    handleSizeChangetal(val) {},
    handleCurrentChangetal(val) {
      this.looksalardetail();
    },
    //添加员工
    addemployees() {
    ...
    //查看工资详情
    looksalary() {
      var url = this.$https.dataHost + "/statistic/salesmanStatistic";
      var params = {
        salesmanID: this.namespeopleId,
        startDate: this.value_invenT[0],
        endDate: this.value_invenT[1]
      };
      this.$https.fetchPost(url, params).then(res => {
        if (res.data.result) {
          this.newdatasalary = res.data.result;
        } else {
          this.$message({
            type: "warning",
    ...
    //查看工资业绩汇总
    looksalarcollect() {
      var url = this.$https.dataHost + "/statistic/yejiHuiZongStatistic";
      var params = {
        salesmanID: this.namespeopleId,
        pageNum: this.currentPagefor,
        pageSize: this.pageSizefor,
        startDate: this.value_invenT[0],
        endDate: this.value_invenT[1]
      };
      this.$https.fetchPost(url, params).then(res => {
        if (res.data.result) {
          this.tableexamine = res.data.result.list;
          this.totalSizesfor = res.data.result.total;
        } else {
          this.tableexamine = [];
          this.totalSizesfor = 0;
    ...
    //查看工资业绩明细
    looksalardetail() {
      var url = this.$https.dataHost + "/statistic/yejiMingXiStatistic";
      var params = {
        salesmanID: this.namespeopleId,
        pageNum: this.currentPagetal,
        startDate: this.value_invenT[0],
        endDate: this.value_invenT[1],
        pageSize: this.pageSizetal
      };
      this.$https.fetchPost(url, params).then(res => {
        if (res.data.result) {
          this.tableexadetail = res.data.result.list;
          this.totalSizestal = res.data.result.total;
        } else {
          this.tableexadetail = [];
          this.totalSizestal = 0;
          this.$message({
            type: "warning",
            message: res.data.responseStatusType.error.errorMsg
          });
    ...
    fetchEmployeesDet(i) {
      ...
      this.part_timeposition = i.parTimePostCategoryName;
      this.employeeeposition = i.postCategoryName;
      this.chanegdtimeposition();
      this.chanegdtiebfn();
      this.chanegdepartment();
      //兼职职务
          ...
          if (res.data.result) {
            ...
            res.data.result.list.forEach(value => {
              this.emlpoyeesData.push({
                ...
                postCategoryName: value.postCategoryName,
                parTimePostCategoryName: value.parTimePostCategoryName
              });
            });
          } else {
            this.$message({
