...
<template>
  <div class="expcardPage">
    ...
    <div class="expxardeCars">
      <div class="somersCar" v-for="item in somersCarshu" :key="item.id">
        ...
        <div class="dislabel">
          ...
          <el-button size="mini" @click="cardparticulars" type="warning">详情</el-button>
        </div>
      </div>
    ...
    <PopOver
      ...
      width="500px"
    >
      <div class="stgblcktop" slot="top">体验卡详情</div>
      <div class="stgblcktopmain" slot="main">
        <div><label>sheng'yu：</label><span>{{salesVolume}}</span></div>
        <div><label>截止日期：</label><span>{{stockNum}}</span></div>
        <div><label>截止日期：</label><span>{{purchaseDeadline}}</span></div>
