...
<template>
  <div class="addpurchase">
    ...
    <div class="invenTitle" v-if="groupOpen == false">
      <h1>出库列表</h1>
      <div id="nc_1_nc-canvas" class="nc-canvas" style="width: 300px; height: 100px;">
        <div
          id="nc_1_bg"
          class="nc-bg"
          style="width: 300px; height: 100px; background: url(&quot;//img.alicdn.com/tps/TB1skE5SFXXXXb3XXXXXXXXXXXX-100-80.png&quot;);"
        >
          <img
            src="//img.alicdn.com/tfs/TB1NYk7SFXXXXcWaXXXXXXXXXXX-57-69.png"
            class="nc-scrape-icon"
            style="left:10px;top:14px;"
          />
          <img
            src="//img.alicdn.com/tfs/TB12q8sSVXXXXcSXFXXXXXXXXXX-57-67.png"
            class="nc-scrape-icon"
            style="left:169px;top:12px;"
          />
        </div>
        <div id="nc_1_cover" class="nc-cover" style="width: 300px; height: 100px;">
          <canvas id="nc_1_canvas" class="nc-canvas-node" width="300" height="100"></canvas>
          <div
            id="nc_1_canvas-dg"
            class="nc-canvas-dg"   
          ...
          ></div>
          <div id="nc_1_ok" class="nc-verify-ok">
            <img src="//img.alicdn.com/tfs/TB1OLQ7SFXXXXaoapXXXXXXXXXX-57-70.png" alt />
            <div>
              <i class="ncpc_iconfont icon_success"></i>
              <span id="nc_1_ok-msg"></span>
            </div>
          </div>
          <div id="nc_1_fail" class="nc-verify-fail">
            <img
              id="nc_1_fail-icon"
              src="//img.alicdn.com/tfs/TB1N4lDSVXXXXcFXpXXXXXXXXXX-57-66.png"
              alt
            />
            <div>
              <i class="ncpc_iconfont icon_error"></i>
              <span id="nc_1_fail-msg"></span>
            </div>
          </div>
          <div id="nc_1_loading" class="nc-loading" style="display: none;">
            ...
            <span>
              <span class="nc-lang-cnt" data-nc-lang="_ggk_loading">加载中</span>
            </span>
          </div>
          <div id="nc_1_inform" class="nc-inform" style="display: block;">
            <span class="nc-lang-cnt" data-nc-lang="_ggk_start"></span>
          </div>
          <div id="nc_1_load-error" class="nc-load-error" style="display: none;">
            <img
              id="nc_1_load-error-icon"
              src="//img.alicdn.com/tfs/TB1N4lDSVXXXXcFXpXXXXXXXXXX-57-66.png"
              alt
            />
            <div>
              <i class="ncpc_iconfont icon_error"></i>
              <span id="nc_1_load-error-msg">Load Error!</span>
            </div>
          </div>
        </div>
        <div id="nc_1_show-how" class="nc-show-how" style="display: none; left: 247px; top: 14px;"></div>
      </div>
              ...
              <el-form :model="addruleForm" :rules="rules" ref="addruleForm" label-width="100px">
                ...
                <el-form-item label="收货方：" prop="addrepertory">
                  ...
                  <div class="seekprbox" v-if="class_selckbox == true">
                    <ul v-for="item in merchantListFilter" :key="item.index">
...
export default {
  ...
  computed: {
    ...
    //模糊查询收货方
    merchantListFilter: function() {
      const input_nurse = this.addruleForm.addrepertory;
      if (input_nurse) {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.class_seekbox == false) {
            if (this.class_selckbox == false) {
