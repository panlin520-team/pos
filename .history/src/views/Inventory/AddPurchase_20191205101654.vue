...
export default {
  ...
  methods: {
    ...
    requcompany() {
      ...
      this.$https.fetchPost(url, params).then(
        res => {
          ...
          console.log(this.option_supplier[0].children[0].supplierCode);
    ...
    //请求供应商列表
    goyisList() {
      var url = this.$https.storeHost + "/manage/supplier/listSupplierCategory";
      var params = {
        storeId: localStorage.getItem("storeId"),
        supplierCategoryId: 1
      };
      this.$https.fetchPost(url, params).then(
        res => {
          ...
          if (res.data.result.list) {
            console.log("请求供应商列表",res.data.result.list);
