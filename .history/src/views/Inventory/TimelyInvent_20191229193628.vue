  ...
  <div class="addpurchase">
    ...
    <div class="invenTitle">
      ...
      <div class="ordersearch">
        <label>商品名称：</label>
        <el-input v-model="inputorder" placeholder="请输入要查询的商品" v-on:keyup.13.native="seekinvtey"></el-input>
    ...
    <div class="clienList">
      <el-table :data="tableData_enterpurchase" style="width: 100%">
        ...
        <el-table-column prop="compile" label="操作">
          <template slot-scope="scope">
            <el-button @click="removeBtn(scope.row)" type="danger" size="mini">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <PopOver
      ...
      :visible.sync="visible_ssqing"
      @close="close_ssqing"
      width="800px"
    >
      <div class="recharge_top" slot="top">退款</div>
      <div class="recharge_main" slot="main">
        <el-table :data="tableData_xainrr" style="width: 100%">
          <el-table-column label="商品名称">
            <template slot-scope="scope">
              <span>{{ scope.row.productName }}</span>
            </template>
          ...
          <!-- <el-table-column prop="compile" label="操作">
            <template slot-scope="scope">
              <el-button @click="removeBtnss(scope.row)" type="danger" size="mini">详情</el-button>
            </template>
          </el-table-column>-->
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    removeBtn() {
      this.input_monely = "";
      this.justinntorysosely();
      this.visible_ssqing = true;
    },
    justinventory() {
    ...
    //即时库存列表
    justinntorysosely() {
      var url = this.$https.productHost + "/stock/selectProductNumberList";
      var params = {
        // pageNum: this.currentPage1,
        // pageSize: this.pageSize,
        stock: localStorage.getItem("stockCode")
      ...
      this.$https
        ...
        .then(res => {
          if (res.data.result) {
            this.tableData_xainrr = res.data.result.list;
          } else {
            this.tableData_xainrr = [];
            this.$message({
              message: res.data.responseStatusType.error.errorMsg,
              type: "warning"
            });
