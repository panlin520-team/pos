    ...
    <div class="merchantList">
      <el-table :data="storageList">
        ...
        <el-table-column label="数量">
          <template slot-scope="scope">
            <div slot="reference" class="name-wrapper">{{ scope.row.stockNum }}</div>
          </template>
        </el-table-column>
        ...
        .then(res => {
          if (res.data.result !== null) {
            ...
            resList.forEach(value => {
              console.log(value.productSubordinate);
              if (value.productSubordinate == 1) {
                value.giveAmount = value.stockQuantity;
              } else {
                value.giveAmount = value.stockNum;
              }
            });
