...
export default {
  ...
  methods: {
    ...
    salaryname(res) {
      log
      // this.namespeopleId = res.name;
      this.visible_times = true;
    },
    //获取时间
    buirdtime() {
      console.log(this.value_invenT[0]);
      console.log(this.value_invenT[1]);
    },
    //确认工资详情
    salary_true() {
      this.visible_salary = true;
                  ...
                  res => {
                    if (res.data.responseStatusType.message == "Success") {
                      ...
                      for (let i = 0; i < arr.length; i++) {
                        const element = arr[i];
                        str += "<p>" + JSON.stringify(arr[i]) + "<p><hr/>";
                      }
                      this.$notify({
                        // 信息
                        message: str,
    ...
    pushService() {
      ...
      for (var i = 0; i < list.length; i++) {
        if (list[i].setEmpId == undefined || list[i].setEmpId == "") {
          ...
          isEmpty = false;
          arr.push({
            beauticianId: list[i].beauticianId,
