          ...
          >
            ...
            >
              <el-tab-pane
                ...
                :key="item.productTypeId2"
                :label="item.productTypeName"
                :name="item.productTypeId2"
              >
                <el-row :gutter="20" class="el_carconnent">
                  <div class="carBox">
                    <div
...
export default {
  ...
  methods: {
    ...
    //商品点击切换
    commonleClis(tab) {
      this.productKindId = tab.name;
      this.projectsubsmall2();
    },
    //分页
      ...
      this.$https
        ...
        .then(res => {
          if (res.data.result.list) {
            ...
            this.commodityTypeID = res.data.result.list[0].commodityTypeID;
            this.projectsubclass(this.commodityTypeID);
          } else {
            this.$message({
      ...
      this.$https
        ...
        .then(res => {
          if (res.data.result) {
            ...
            console.log(res.data.result.list);
            this.subclassID = res.data.result.list[0].subclassID;
      ...
      this.$https
        ...
        .then(res => {
          if (res.data.result) {
            ...
            for (let i = 0; i < arr.length; i++) {
              arr[i].productTypeId2 = arr[i].productTypeId + "";
              arr[i].productTypeId = arr[i].productTypeId + "";
            }
            this.productbleTabdise = res.data.result;
            // this.projectsubsmall(res.data.result.list[0].commodityTypeID)
          } else {
            this.productbleTabdise = [];
            this.$message({
              message: res.data.responseStatusType.error.errorMsg,
              type: "warning"
            });
