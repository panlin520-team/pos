  ...
  <div class="customization-page_root">
    <el-row>
      ...
      <div class="elleft">
        ...
        <div v-for="item in tableDataListss" :key="item.name">
          <div style="width:100px;heigth:100px;border:1px solid red">{{item.productName}}</div>
        </div>
        <!-- 大类 -->
...
export default {
  ...
  methods: {
    ...
    //     this.tableDataList.push(this.chanpinlist);
    //   } else {
    //     // this.tableDataList.push(this.chanpinlist);
    //     this.setlist();
    //   }
    ...
    //项目
    // xiangmu() {
    //   // this.tableDataLists.push(this.xiangmulist);
    //   // this.setlists();
    // },
    //列表去重
    setlist: function() {
      // var arr = this.tableDataLists;
      // var result = [];
      // var obj = {};
      // for (var i = 0; i < arr.length; i++) {
      //   if (!obj[arr[i].productCode]) {
      //     result.push(arr[i]);
      //     obj[arr[i].productCode] = true;
      //   }
      // }
      ...
      var ress = this.tableDataList.find(item => {
        ...
        console.log(this.chanpinlist.productCode);

        return item.productCode == this.chanpinlist.productCode;
      });
      console.log("ress", ress);
      if (ress.productCode !== this.chanpinlist.productCode) {
        this.tableDataList.push(this.chanpinlist);
        console.log(this.tableDataList);
      } else {
        this.$message({
    ...
    setlists: function() {
      ...
      // console.log(this.tableDataList);
      var ress = this.tableDataList.find(item => {
        item.productCode == "wayesdgda";
      });
      console.log("ress", ress);
      if (ress !== undefined) {
        this.tableDataList.push(this.xiangmulist);
      } else {
        this.$message({
          message: "该商品已存在，可直接修改数量",
          type: "warning"
    ...
    car_smreos(res) {
      ...
      //   }
      // } else {
      //   var res = this.tableDataList.find(item => {
      //     return console.log(item.productCode);
      //   });
