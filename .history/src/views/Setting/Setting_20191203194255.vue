  ...
  methods: {
    ...
    init: function() {
      var map = new window.AMap.Map("container", {
        ...
        zoom: 16, //初始化地图层级
        center: [this.GaodeLongitude, this.GaodeLongitude] //初始化地图中心点
      });
    },
    // 修改门店信息
    saveStore() {
    ...
    fetchStore() {
      ...
      this.$https
        .fetchPost(url, params)
        .then(res => {
          console.log(res.data.result[0].gaodeLatitude);
          console.log(res.data.result[0].gaodeLongitude);
          if (res.data.result) {
            this.GaodeLongitude = res.data.result[0].gaodeLongitude;
            this.GaodeLatitude = res.data.result[0].gaodeLatitude;
          ...
          console.log(this.GaodeLatitude);
          console.log(this.GaodeLongitude);
        })
        .catch(err => {});
    },
    // 获取经营行业数据
